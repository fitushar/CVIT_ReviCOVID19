...
def main(model_name: str = "efficientnetv2-s",
    ...
    torch_weights_path="/Local/results/efficientnetv2-s-21k-torch/"
    if os.path.exists(torch_weights_path) is False:
        #os.makedirs("./weights")
        os.makedirs(torch_weights_path)
    torch.save(new_weights, torch_weights_path +  "pre_new_new" + model_name + ".pth")


if __name__ == '__main__':

    main(model_name="efficientnetv2-s",
        tf_weights_path="/Local/results/efficientnetv2-s-21k/",
        stage0_num=2,
        fused_conv_num=10)
